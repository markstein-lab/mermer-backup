...
//Copyright (c) in silico Labs, LLC 2009, 2105
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include "scanner.h"
...
void outputGraphicBetween(int i, int pos, int posend) {
    ...
    if (i == 0) i = 1;  //temporary fix if cluster to the left of first gene
...
//Row2
    ...
    printf("<td align='center'> %s</td>\n", 
       s = formatNumber(rightStop - leftStart + 1));
...
//Row 3
    ...
    printf("<td height='5' bgcolor='navy'>\n");
...
//        fileName, clusterNumber);
    printf("<img height='3' width='%ld'></A></td>", 
             (centerPart > 9*strlen(s))?centerPart : 9*strlen(s));
...
//Row3
    for (n = 0; n < map[i].numberOfmRNAs; n++) {
       ...
       used = 0; 
       if(MRNA(i, n, 4) > map[i].start) {
          used = (MRNA(i, n, 4) - map[i].start) * 500 / distance;
          printf("<td width = '%d' height='5'></td>\n", used);
       }
       for (k = 1; k <= spaces; k++) {
          sofar = (MRNA(i, n, 4+k) - map[i].start) * 500 / distance;
          this = sofar - used;
          if (this == 0) this++;
          if (k & 1) { //exon
...
//Row4
    printf("<table cellpadding='0' cellspacing='0' border='0'>");
    printf("<tr>\n");
    printf("<td width='%d' align='center'><font size='2'>%s</font></td>\n",
        leftPart, formatNumber(pos - map[i].start));
    printf("<td height='5' bgcolor='navy' >\n");
    printf("<A id='control%d' HREF=\"javascript:toggle('content%d');\">",
        clusterNumber, clusterNumber);
//    printf("<A HREF='/cgi-bin/showcluster.cgi?FN=%s&CN=%d' target='_blank'>",
//        fileName, clusterNumber);
    printf("<img height='5' width='%d' ></A></td>", centerPart);
    printf("</td>\n");
    printf("<td width='%d' align='center'><font size='2'>%s</font></td></tr>", 
        rightPart, formatNumber(map[i].stop - posend));
/******
//Row 5
    printf("<tr><td rowspan='2' align='right'>%s</td><td></td></tr>\n",
